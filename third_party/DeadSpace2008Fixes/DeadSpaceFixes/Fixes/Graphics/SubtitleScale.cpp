...
#pragma pack(pop)
			...
			void __stdcall LogSubtitleFunc(void* eax, int ecx, uint32_t stackParam) {
				...
				// Prefer the actual Dead Space render window over the desktop mode.
				// This matters for exclusive fullscreen and low-resolution testing.
				...
				// The stock 720p scale is also the minimum. Resolutions below 720p
				// must not make subtitles smaller than the original game.
				float subtitleMultiplier = static_cast<float>(screenHeight) / 720.0f;
				if (subtitleMultiplier < 1.0f)
