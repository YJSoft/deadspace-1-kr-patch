...
output="${1:-$repo_root/dist/DeadSpace1-KR-0.2-x86_64.AppImage}"
...
for required in \
  ...
  "$repo_root/packaging/patches/deadspace1-kr-v0.2.pat" \
...
install -m 0644 "$repo_root/packaging/patches/deadspace1-kr-v0.2.pat" \
  "$appdir/usr/share/deadspace1-kr/patches/"
...
rm -f "$output"
ARCH=x86_64 VERSION=0.2 "$appimagetool" --appimage-extract-and-run \
