...
`deadspace1-kr-v0.1.pat`는 Steam판 Dead Space (2008) 1.0.0.222의 원본 리소스를
한국어 패치 리소스로 변환하는 VPatch 데이터다. 원본 또는 완성 STR 자체는 저장소와
설치 파일에 포함하지 않는다. 현재 Steam 영어 설치본의 `12F4D5F8.str`를 입력으로
받아 패치용 `D8CBB618.str`를 새로 만든다.
...
있게 만드는 호환 입력이다. 둘 다 생략하거나 둘 다 지정해야 한다. 실제 Steam 순정
입력의 SHA-256은 필수로 확인하며, `manifest.json`에 지원 입력을 모두 기록한다.
