...
5. 원본 백업 일부를 제거한 채 업그레이드하면 게임 파일 변경 없이 중단되는지 확인한다.
   그 상태에서 해당 게임 클라이언트로 원본 STR를 복원해 재실행하고 백업 재구성과
   설치를 확인한다.
...
Steam판의 `Application load error 5:0000065434`는 패치 훅 오류가 아니라 Steam DRM
실행 경로 문제일 수 있다. 게임 EXE를 직접 더블클릭하지 말고 Steam 라이브러리에서
실행한다.
