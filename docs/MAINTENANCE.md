...
4. 같은 폴더에 재설치한 뒤 제거하여 원본 STR와 런타임 파일이 정확히 복원되는지
   해시로 확인한다.
5. 잘못된 STR에서 설치가 변경 없이 중단되는지 확인한다.
6. 실제 게임에서 `docs/TESTING.md`의 기본/회귀 항목을 실행한다.
7. 다음 라이선스 전문을 설치 파일에 넣는다.
   ...
   - NSIS 라이선스
   - VPatch zlib
8. DLL과 완성 STR는 Git 소스 브랜치에 커밋하지 않는다. 배포는 CI가 생성한
