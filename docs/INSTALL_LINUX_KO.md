...
패치 0.2를 설치한다. Ubuntu 26.04와 Steam Deck 데스크톱 모드에서 같은 AppImage를
사용할 수 있다.
...
1. Steam에서 Dead Space를 설치하고 한 번도 실행하지 않았더라도 게임 파일이 완전히
   내려받아졌는지 확인한다.
2. 내려받은 `DeadSpace1-KR-0.2-x86_64.AppImage`에 실행 권한을 준다.

   ```bash
   chmod +x DeadSpace1-KR-0.2-x86_64.AppImage
   ```
...
```bash
./DeadSpace1-KR-0.2-x86_64.AppImage --appimage-extract-and-run
```
